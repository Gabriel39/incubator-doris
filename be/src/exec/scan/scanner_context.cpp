...
// ==================== ScannerContext ====================
ScannerContext::ScannerContext(RuntimeState* state, pipeline::ScanLocalStateBase* local_state,
...
std::string ScannerContext::debug_string() {
    return fmt::format(
            ...
            print_id(_query_id), ctx_id, _all_scanners.size(), _tasks_queue.size(), _should_stop, _is_finished,
...
int32_t ScannerContext::_get_margin(std::unique_lock<std::mutex>& transfer_lock,
    ...
    int32_t margin_3 = effective_max_concurrency -
                       (cast_set<int32_t>(_tasks_queue.size()) + _num_scheduled_scanners);

    if (margin_1 <= 0 && margin_2 <= 0) {
        return _enable_adaptive_scanners ? std::max(0, margin_3)
                                         : 0; // Still respect adaptive limit
    }

    int32_t margin = std::max(margin_1, margin_2);
    margin = std::min(margin, std::max(0, margin_3)); // Cap by adaptive limit
    ...
    VLOG_DEBUG << fmt::format(
            ...
            _scanner_scheduler->get_active_threads(), _scanner_scheduler->get_queue_size(),
            margin_3, effective_max_concurrency, _tasks_queue.size(), _num_scheduled_scanners,
